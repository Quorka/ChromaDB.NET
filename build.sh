...
cargo build --release
cp "$ROOT_DIR/target/release/libchroma_csharp.so" "$OUTPUT_DIR/linux-x64/native/"

# Cross-compile for Windows (requires appropriate target)
if rustup target list --installed | grep -q "x86_64-pc-windows-msvc"; then
    echo "Building for Windows x64..."
    cargo build --release --target x86_64-pc-windows-msvc
    cp "$ROOT_DIR/target/x86_64-pc-windows-msvc/release/chroma_csharp.dll" "$OUTPUT_DIR/win-x64/native/"
else
...
if rustup target list --installed | grep -q "x86_64-apple-darwin"; then
    ...
    cp "$ROOT_DIR/target/x86_64-apple-darwin/release/libchroma_csharp.dylib" "$OUTPUT_DIR/osx-x64/native/"
else
...
if rustup target list --installed | grep -q "aarch64-apple-darwin"; then
    ...
    cp "$ROOT_DIR/target/aarch64-apple-darwin/release/libchroma_csharp.dylib" "$OUTPUT_DIR/osx-arm64/native/"
else
